...
//옳바른 형식의 이메일만 적절한 컬렉션에 저장(몇개인지 카운트)
//중복되는 이메일이 있다면 무엇인지 찾아보세요

public class D09_CollectEmail {
	static ArrayList<String> list = new ArrayList<>();
	static ArrayList<String> overlap = new ArrayList<>();
	...
	static void emailCheck(String email) {
		...
		for(int i=0; i<list.size(); i++) {
			if(Collections.frequency(list, list.get(i))>1) {
				overlap.add(list.get(i));
			}						
		}
		
		HashSet<String> overlap2 = new HashSet<>(overlap);
		System.out.println("중복된 이메일 개수:"+overlap2.size());
		for(String overlapEmail : overlap2) {
			System.out.println(overlapEmail);
		}
	}
